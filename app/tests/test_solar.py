...
def test_equator_has_about_twelve_hours_at_equinox():
    events = solar.day_events(EQUINOX, *QUITO, tz=-5.0)
    assert 700 <= events["daylight_minutes"] <= 740


def test_lofoten_has_midnight_sun_in_june():
    events = solar.day_events(JUNE, *LOFOTEN, tz=2.0)
...
def test_tromso_has_polar_night_in_december():
    events = solar.day_events(DECEMBER, *TROMSO, tz=1.0)
...
def test_oslo_june_is_much_longer_than_oslo_december():
    summer = solar.day_events(JUNE, *OSLO, tz=2.0)
    winter = solar.day_events(DECEMBER, *OSLO, tz=1.0)
    assert summer["daylight_minutes"] > winter["daylight_minutes"] + 600


def test_southern_hemisphere_seasons_invert():
    june = solar.day_events(JUNE, *SYDNEY, tz=10.0)
    december = solar.day_events(DECEMBER, *SYDNEY, tz=11.0)
    assert december["daylight_minutes"] > june["daylight_minutes"]


def test_sun_is_higher_at_the_equator_than_in_oslo_at_equinox():
    equator = solar.day_events(EQUINOX, *QUITO, tz=-5.0)
    oslo = solar.day_events(EQUINOX, *OSLO, tz=1.0)
...
def test_events_are_reported_in_local_time():
    ...
    events = solar.day_events(JUNE, 36.4618, 25.3753, tz="Europe/Athens")
...
def test_local_noon_is_near_the_middle_of_the_day():
    events = solar.day_events(JUNE, *OSLO, tz=2.0)
...
def test_named_zones_follow_daylight_saving():
    ...
    summer = solar.day_events(JUNE, *OSLO, tz="Europe/Oslo")
    winter = solar.day_events(DECEMBER, *OSLO, tz="Europe/Oslo")
...
def test_southern_daylight_saving_also_inverts():
    ...
    june = solar.day_events(JUNE, -50.9423, -73.4068, tz="America/Santiago")
    december = solar.day_events(DECEMBER, -50.9423, -73.4068, tz="America/Santiago")
