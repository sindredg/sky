...
async function loadPlaces(){
  if(state.places.length||state.placesError)return;
  try{
...
function refreshPlaceDetail(){
  ...
  if(day===state.placeDay)return;
  clearTimeout(detailTimer);
  detailTimer=setTimeout(()=>loadPlaceDetail(place.slug,placeDay(place,state.date)),300);
}
...
function updateInspector(full=true){
  ...
  if(full){
    ...
    $('portrait').setAttribute('aria-label',state.view==='sky'?`Illustration of ${item.name}`:state.view==='scale'?'Earth as a reference for distance':state.view==='earth'?'Earth, lit as it is now':`Artistic illustration of ${item.name}`);
  }
  if(state.view==='solar'){
    ...
    const failed=state.placeData==='error',light=failed?null:state.placeData?.light,moon=failed?null:state.placeData?.moon;
    if(full){
      $('object-type').textContent=`${item.country} · ${item.timezone}`;$('object-symbol').textContent='⌖';
      $('portrait-caption').textContent='THE DAY SIDE OF THE WORLD';
    ...
    $('light-copy').innerHTML=failed?'The server could not be reached, so tonight’s times are unavailable.':milkyCopy(state.placeData?.milky);
  }else{
...
function updateObjects(){
  ...
  if(state.view==='earth'){
    ...
    const rows=state.places.map(place=>({place,phase:lightPhase(altitudeFrom(sun,place.latitude,place.longitude))})).sort((a,b)=>rank[a.phase]-rank[b.phase]||a.place.name.localeCompare(b.place.name));
    for(const {place,phase} of rows){
      ...
      button.innerHTML=`<span class="mini-planet star" style="--planet-color:${phaseColors[phase]}"></span><span><strong>${escapeHtml(place.name)}</strong><small>${phaseNames[phase]}</small></span>`;
...
function setDate(date,reanchor=true){state.date=clampDate(date);state.live=false;state.playing=false;if(reanchor)anchor=state.date;updateTime();if(state.view==='sky'||state.view==='earth'){updateObjects();refreshPlaceDetail();}renderer.draw();}
